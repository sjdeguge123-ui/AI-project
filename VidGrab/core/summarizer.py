...
def _language_instruction(language: str) -> str:
    ...
    if lang == "ja":
        return (
            "すべてを日本語で出力し、日本語として自然な句読点（、。など）で適切に区切ってください。"
            "章の要点・内容のまとめ・名言（golden quotes）はすべて日本語としてください。"
            "中国語や英語に翻訳しないでください（ただし、中国語・英語由来の固有名詞は原文のままでかまいません）。"
        )
    if lang == "ko":
        return (
            "모든 것을 한국어로 출력하고, 한국어에 자연스러운 문장 부호(,. 등)로 적절히 끊어 쓰세요. "
            "챕터 요점·내용 요약·명언(golden quotes)은 모두 한국어로 작성하세요. "
            "중국어나 영어로 번역하지 마세요(단, 중국어·영어 고유명사는 원문 그대로 둬도 됩니다)."
        )
    ...
    return (
        ...
        "若主要是日文/韩文，则分别用日文/韩文输出。"
        "不要混合语言，不要擅自把内容翻译成其他语言。"
    )
